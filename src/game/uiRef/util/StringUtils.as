package  game.uiRef.util
{
	...
	public class StringUtils
	{
		...
		public static function createLinkPath(pngPath:String):String
		{
			return "<font color=\"#0000ff\"><a href=\"event:" + pngPath + "\">" + pngPath + "</a></font>";
		}
		public static function replace$value$(rawStr:String, ... args):String
		{
		...
		public static function replace$value$2(rawStr:String,replacePart:String, replaceTarget:String):String
		{
			var reg:RegExp=new RegExp(replacePart,"g")
			return rawStr.replace(reg, replaceTarget);
		}
		public static function getFirstLowerString(rawStr:String):String
		{
			return rawStr.charAt(0).toLocaleLowerCase() + rawStr.substr(1);
		}
		/**
		...
		public static function truncate(p_string:String,p_len:uint,p_suffix:String="..."):String
		{
			...
			if (trunc.length > p_len)
			{
				...
				if (RegExp(/[^\s]/)['test'](p_string.charAt(p_len)))
				{
		...
		public static function removeAllSpace(p_string:String):String
		{
			if ((p_string == null))
			{
				return '';
		...
		//----------------------------------------------
		public function ltrimNumber(str:String):Boolean
		{
			var pattern:RegExp = /^\d+(\.\d{0,2}){0,1}$/;
			var result:Object = pattern.exec(str);
			if (((result == null) || Number(str) == 0))
			{
				return false;
			}
			return true;
		...
		//去除指定字符串;  
		public static function remove1(char:String,remove:String):String{  
