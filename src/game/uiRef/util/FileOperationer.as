...
//		}
		...
		public static function getAllFiles(root:File, extName:String):Vector.<File> {
			var ret:Vector.<File> = new Vector.<File>;
			readToList(root, ret, new FileExtNameFilter(extName));
				 
			return ret;
		}
			
		public static function  getAllFileByFilter( root:File,  filter:Filter):Vector.<File> {
			var ret:Vector.<File> = new Vector.<File>;;
		...
		public static function readToList( root:File, ret:Vector.<File>,filter:Filter) :void {
			
			if (root.isDirectory) {
				var subFiles:Array = root.getDirectoryListing();
				if (subFiles != null) {
